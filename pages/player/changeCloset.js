...
function initUI(){
    ...
    ui_btn_next.position.set(202.5,110,202.5)
    ...
    PLAYER_LOADER.LoadEquipList(ActivePart, 1).then(e => {
        ActivePart = 0;
        loadPage(e,0)
    });
    ...
    let category = ["Hat", "Hair", "Shirt", "Pants", "Shoes"]
    for (let i = 0; i < category.length; i++) {
        let plane = new THREE.PlaneGeometry(35,45.5)
        let eyeTexture = TEXTURE_LOADER.load('../../texture/ui/equip_'+category[i]+'.png')
        ...
        mesh.name = "category_" + i
        mesh.position.set(232,210-(i*45),232)
            ...
            if(event.which == 1){
                ...
                items.forEach(i=>{
                    ...
                    if(obj_name.startsWith("button_")){
                        switch(obj_name){
                            case "button_next":
                                PLAYER_LOADER.LoadEquipList(ActivePart,PAGE+1).then(e => {
                                    loadPage(e,1)
                                })
                                break;
                            case "button_prev":
                        ...
                        switch(parseInt(obj_name.charAt(9))){
                            case 0: 
                                ActivePart = 0
                            break;
                            case 1: 
                    ...
                    } else if(obj_name.startsWith("equip_")){
                        console.log("Equipping "+ obj_name)
                        SCENE.remove(PLAYER_PREVIEW)
                        let ppreview_temp = PLAYER_LOADER.EquipItem(parseInt(obj_name.charAt(6)),ActivePart)
                        PLAYER_PREVIEW = ppreview_temp.PLAYER
                        equippedItem.push(ppreview_temp.equippedItem)
                        PLAYER_PREVIEW.position.set(-57.5,36,-37.5)
                        addEquipped(i.object)
                        SCENE.add(PLAYER_PREVIEW)
                    } else if(obj_name.startsWith("equipped_")){
                        SCENE.remove(PLAYER_PREVIEW)
                        PLAYER_PREVIEW = PLAYER_LOADER.UneqItem(equippedItem[parseInt(obj_name.charAt(9))])
                        equippedItem.splice(parseInt(obj_name.charAt(9)),1)
                        PLAYER_PREVIEW.position.set(-57.5,36,-37.5)
                        removeEquipment(parseInt(obj_name.charAt(9)))
                        SCENE.add(PLAYER_PREVIEW)
                    }
                })
            }
...
function removeEquipment(num){
    equippedList.forEach(e => {
        UI.remove(e)
    })
    equippedList.splice(num,1)
    for (let i = 0; i < equippedList.length; i++) {
        equippedList[i].position.set(-50+(i*25),-50,-50+(i*25))
        let temp = equippedList[i].name
        equippedList[i].name = "equipped_"+i+"_"+temp.substring(11,temp.length)
...
function addEquipped(obj){
    ...
    for (let i = 0; i < equippedList.length; i++) {
        equippedList[i].position.set(-50+(i*25),-50,-50+(i*25))
        equippedList[i].name = "equipped_"+i+"_"+parseInt(obj.name.charAt(6))+"_"+ActivePart
        UI.add(equippedList[i])
    }
