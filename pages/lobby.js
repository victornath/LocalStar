...
function load(){
    ...
    for (let i = 1; i <= 3; i++) {
        TEXTURE_LOADER.load('../../texture/ui/game_'+i+'.png', function(texture){
            let map = "game_"+i
            LOADED_TEXTURE[map] = texture
        });
    }
    TEXTURE_LOADER.load('../../texture/ui/help_btn.png', function(texture){
        LOADED_TEXTURE["help"] = texture
    });
    TEXTURE_LOADER.load('../../texture/ui/setting_btn.png', function(texture){
        LOADED_TEXTURE["setting"] = texture
    });
    TEXTURE_LOADER.load('../../texture/ui/edit_avatar_clicked.png', function(texture){
        LOADED_TEXTURE["edit_avatar_click"] = texture
    });
    let button = ["appearance", "closet"]
    for (let i = 0; i < 2; i++) {
        TEXTURE_LOADER.load('../../texture/ui/edit_'+button[i]+'.png', function(texture){
            LOADED_TEXTURE[button[i]] = texture
        });
    }
}
...
function initUI(){
    ...
    let bottom_menu = ["Shop","Inventory","Ranking"]
    for (let i = 0; i < 3; i++) {
    ...
    const name_geometry = new TextGeometry( "Player Name", {
        font: LOADED_FONT,
        size: 12,
        height: 0,
        bevelEnabled: false,
                    ...
                    if(obj_name.startsWith("help_")){
                        ...
                        switch(choice){
                            case 0:
                                ...
                                break;
                            case 1:
                                window.open("../pages/inventory.html", "_self")
                                break;
                            case 2:
